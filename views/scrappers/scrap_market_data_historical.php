				...
				if(count($market_json->cash) > 0) {
					...
					<tr<?=$i%2?' class="row2"':""?>>
					    ...
					    */ ?>
					    ...
					    <td><span class = "history_button" data-symbol = "<? echo $m->symbol ?>" data-change-amount = "$<?=$m->change?>" data-change-percent = "<?=$m->change_percent?>%" data-change-cost = "$<?=$m->last ?>" data-change-class = "<?=$extra_class?>" data-chart-title = "<?=$m->material ?>">history</span></td>
					</tr>
					<?
					$i++;
					}
				} else { ?>
					<tr><td colspan = "6" style ="padding-top: 80px; text-align: center;">There was an error loading the data</td></tr>
			<?php }
				?>
			</table>
		</div>
